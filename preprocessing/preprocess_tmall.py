...
def preprocess_org( path=DATA_PATH, file=DATA_FILE, path_proc=DATA_PATH_PROCESSED, version=VERSION, min_item_support=MIN_ITEM_SUPPORT, min_session_length=MIN_SESSION_LENGTH ):
    ...
    data = load_data_prepared( path+file, version )
...
def preprocess_org_min_date( path=DATA_PATH, file=DATA_FILE, path_proc=DATA_PATH_PROCESSED, version=VERSION, min_item_support=MIN_ITEM_SUPPORT, min_session_length=MIN_SESSION_LENGTH, min_date=MIN_DATE, days_test=DAYS_TEST ):
...
#just load and show info
def preprocess_save( path=DATA_PATH, file=DATA_FILE, path_proc=DATA_PATH_PROCESSED, version=VERSION, min_item_support=MIN_ITEM_SUPPORT, min_session_length=MIN_SESSION_LENGTH ):
...
def load_data( file, version ) : 
    ...
    if( version == 15 ):
        #load csv
        data = pd.read_csv( file+'.csv', sep=',', header=0, usecols=[0,1,5,6], dtype={0:np.int32, 1:np.int32, 5:str, 6:np.int32} )
...
def split_data( data, output_file, days_test ) :
    ...
    print('Full train set\n\tEvents: {}\n\tSessions: {}\n\tItems: {}\n\tSpan: {} / {}'.format(len(train), train.SessionId.nunique(), train.ItemId.nunique(), data_start, data_end) )
    train.to_csv(output_file + '_train_full.txt', sep='\t', index=False)
    
    data_start = datetime.fromtimestamp( test.Time.min(), timezone.utc )
    data_end = datetime.fromtimestamp( test.Time.max(), timezone.utc )
    print('Test set\n\tEvents: {}\n\tSessions: {}\n\tItems: {}\n\tSpan: {} / {}'.format(len(test), test.SessionId.nunique(), test.ItemId.nunique(), data_start, data_end) )
    test.to_csv(output_file + '_test.txt', sep='\t', index=False)
