...
if __name__ == '__main__':
    ...
    Configuration
    '''
    data_path = 'data/8tracks/slices/'
    file_prefix = 'playlists-8tracks'
    ...
    export_csv_base = 'results/test-results-window'
    
    for slice in range(5):
        
        print('process slice '+str(slice))
        
        export_csv = export_csv_base + str(slice) + '.csv'
        ...
        metric.append(pop.Popularity(1))
        ...
        vsknna = vsknn.VMContextKNN( 100, 2000, similarity="cosine", last_n_days=None, extend=False )
        algs['vsknn-200-2000-cosine'] = vsknna
         
        ssknna = ssknn.SeqContextKNN( 100, 500, similarity="cosine", extend=False )
        algs['ssknn-100-500-cosine-div'] = ssknna
         
        sfsknna = sfsknn.SeqFilterContextKNN( 100, 500, similarity="cosine", extend=False )
        algs['sfsknn-100-500-cosine-div'] = sfsknna
        ...
        smfa = smf.SessionMF( factors=100, batch=32, learn='adagrad_sub', learning_rate=0.085, momentum=0.2, regularization=0.005, dropout=0.3, skip=0.0, epochs=10, shuffle=-1, activation='linear', objective='top1_max' )
        algs['smf-top1-max'] = smfa
    ...
    #     algs['fm-gru'] = fmdl;
        ...
        hybrid = wh.WeightedHybrid( [vsknna, sra], [0.5,0.5], fit=False )
...
#         if gru != None:
#             file = open('mdl/' + data_path.replace('/', '_') + '_' + 'gru4rec2-bpr-max_slice' + str(slice) + '.mdl', 'wb+')
#             pkl.dump(gru, file)
#             file.close()
